//
//  AdPopUpLaunchViewController_iPad.m
//  Parity Plus
//
...
@interface AdPopUpLaunchViewController_iPad ()

@end

@implementation AdPopUpLaunchViewController_iPad
...
- (UIViewController *)viewControllerAtIndex:(NSUInteger)index {
    ...
    {
        self.boxChildViewController = [[AdViewController_iPad alloc] initWithNibName:@"AdViewController_iPad" bundle:nil];
...
- (UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerBeforeViewController:(UIViewController *)viewController {
    ...
    if (pageViewController == self.pageController) {
        ...
        if ([currentView isKindOfClass:[AdViewController_iPad class]]) {
            
            index = [(AdViewController_iPad *)viewController index];
        }
...
- (void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray *)previousViewControllers transitionCompleted:(BOOL)completed {
    ...
    if (pageViewController == self.pageController) {
        ...
        if ([currentView isKindOfClass:[AdViewController_iPad class]]) {
            ...
            if ([(AdViewController_iPad *)currentView index] == -1)
            {
                self.myPageControl.currentPage = self.adFeeds.count-1;
            }
            else
            {
                self.myPageControl.currentPage = [(AdViewController_iPad *)currentView index];
            }
